...
from common import Logger, ParallelProcessor
...
def test_parallel_processing_logic():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert (
            all_files_in_batches == 25
        ), f"Expected 25 files in batches, got {all_files_in_batches}"
...
def test_batch_optimization():
    ...
    for concurrency_input, expected_max in test_cases:
        ...
        assert (
            1 <= batch_size <= expected_max
        ), f"Batch size {batch_size} not in range 1-{expected_max}"
...
def test_file_discovery():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for extensions, expected_count in test_cases:
            ...
            ), f"Expected {expected_count}, got {len(files)} for extensions '{extensions}'"
...
def test_report_merging():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert (
            merged["urls"]["validated"] == 25
        ), f"Expected 25 validated URLs, got {merged['urls']['validated']}"
        assert (
            merged["files"]["total"] == 8
        ), f"Expected 8 total files, got {merged['files']['total']}"
